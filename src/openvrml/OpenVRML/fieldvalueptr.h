...
namespace OpenVRML {
    ...
    public:
        explicit FieldValuePtr(field_value * fieldValue = 0);
        ...
        field_value & operator*() const;
        field_value * operator->() const;
        field_value * get() const;

        void reset(field_value * fieldValue = 0);
    ...
    inline bool
    FieldValuePtr::operator==(const FieldValuePtr & fieldValuePtr) const
    {
        return (this->fieldValue == fieldValuePtr.fieldValue);
    }

    inline field_value & FieldValuePtr::operator*() const
    {
    ...
    inline field_value * FieldValuePtr::get() const
    {
        assert(this->fieldValue);
        return this->fieldValue;
    }
