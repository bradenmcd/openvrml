...
namespace openvrml {
    ...
    enum vrml_parse_error {
        ...
        vec2f_expected,
        vec2f_or_lbracket_expected,
        vec2f_or_rbracket_expected,
        vec3f_expected,
        vec3f_or_lbracket_expected,
        vec3f_or_rbracket_expected,
        node_expected,
    ...
    const vrml_parse_assertion expect_image(image_expected);
    ...
    const vrml_parse_assertion expect_vec2f(vec2f_expected);
    const vrml_parse_assertion expect_vec3f(vec3f_expected);
    ...
    struct event_interface_type_parser {
        typedef node_interface::type_id result_t;
                                  ...
                                  result_t & result) const
        {
            ...
            match_t match
                =   (   str_p("eventIn")[
                            var(result) = node_interface::eventin_id
                        ]
                    |   str_p("eventOut")[
                            var(result) = node_interface::eventout_id
                        ]
                    ).parse(scan);
            return match.length();
        }
    ...
    const boost::spirit::functor_parser<event_interface_type_parser>
        event_interface_type_p;


    struct field_interface_type_parser {
        typedef node_interface::type_id result_t;
                                  ...
                                  result_t & result) const
        {
            ...
            match_t match
                =   (   str_p("field")[
                            var(result) = node_interface::field_id
                        ]
                    ).parse(scan);
            return match.length();
        }
    ...
    const boost::spirit::functor_parser<field_interface_type_parser>
        field_interface_type_p;


    struct exposedfield_interface_type_parser {
        typedef node_interface::type_id result_t;
                                  ...
                                  result_t & result) const
        {
            ...
            match_t match
                =   (   str_p("exposedField")[
                            var(result) = node_interface::exposedfield_id
                        ]
                    ).parse(scan);
            return match.length();
        }
    ...
    const boost::spirit::functor_parser<exposedfield_interface_type_parser>
        exposedfield_interface_type_p;


    struct interface_type_parser {
        typedef node_interface::type_id result_t;
                                  ...
                                  result_t & result) const
        {
            ...
            typedef typename match_result<ScannerT, result_t>::type match_t;
            match_t match
                =   (   event_interface_type_p[var(result) = arg1]
                    |   field_interface_type_p[var(result) = arg1]
                    |   exposedfield_interface_type_p[var(result) = arg1]
                    ).parse(scan);
            return match.length();
        }
    };

    const boost::spirit::functor_parser<interface_type_parser>
        interface_type_p;


    struct field_type_parser {
        typedef field_value::type_id result_t;
                                  ...
                                  result_t & result) const
        {
            ...
            match_t match
                =   (   str_p("MFColor")[
                            var(result) = field_value::mfcolor_id
                        ]
                    |   str_p("MFFloat")[
                            var(result) = field_value::mffloat_id
                        ]
                    |   str_p("MFInt32")[
                            var(result) = field_value::mfint32_id
                        ]
                    |   str_p("MFNode")[
                            var(result) = field_value::mfnode_id
                        ]
                    |   str_p("MFRotation")[
                            var(result) = field_value::mfrotation_id
                        ]
                    |   str_p("MFString")[
                            var(result) = field_value::mfstring_id
                        ]
                    |   str_p("MFTime")[
                            var(result) = field_value::mftime_id
                        ]
                    |   str_p("MFVec2f")[
                            var(result) = field_value::mfvec2f_id
                        ]
                    |   str_p("MFVec3f")[
                            var(result) = field_value::mfvec3f_id
                        ]
                    |   str_p("SFBool")[
                            var(result) = field_value::sfbool_id
                        ]
                    |   str_p("SFColor")[
                            var(result) = field_value::sfcolor_id
                        ]
                    |   str_p("SFFloat")[
                            var(result) = field_value::sffloat_id
                        ]
                    |   str_p("SFImage")[
                            var(result) = field_value::sfimage_id
                        ]
                    |   str_p("SFInt32")[
                            var(result) = field_value::sfint32_id
                        ]
                    |   str_p("SFNode")[
                            var(result) = field_value::sfnode_id
                        ]
                    |   str_p("SFRotation")[
                            var(result) = field_value::sfrotation_id
                        ]
                    |   str_p("SFString")[
                            var(result) = field_value::sfstring_id
                        ]
                    |   str_p("SFTime")[
                            var(result) = field_value::sftime_id
                        ]
                    |   str_p("SFVec2f")[
                            var(result) = field_value::sfvec2f_id
                        ]
                    |   str_p("SFVec3f")[
                            var(result) = field_value::sfvec3f_id
                        ]
                    ).parse(scan);
            return match.length();
        }
    ...
    struct interface_decl_parser {
        ...
        operator()(const ScannerT & scan, result_t & result) const
        {
            ...
            assertion<vrml_parse_error>
                expect_field_type(field_type_expected),
                expect_id(id_expected);

            rule_t interface_decl_rule
                =   interface_type_p[var(result.type) = arg1]
                    >> expect_field_type(
                            field_type_p[var(result.field_type) = arg1]
                        )
                    >> expect_id(id_p[var(result.id) = arg1])
                ;
            BOOST_SPIRIT_DEBUG_NODE(interface_decl_rule);

            match_t match = interface_decl_rule.parse(scan);
            return match.length();
        }
    };
                                  ...
                                  result_t & result) const
        {
            ...
            rule_t rule
                =   int32_p[var(pixels) = arg1, var(x) = arg1]
                    ...
                    >> repeat_p(ref(pixels))[
                        int32_p[set_pixel(result, index)][var(index) += 1]
                    ]
                ;
            match_t match = rule.parse(scan);
            return match.length();
    ...
    struct vrml97_grammar :
        ...
        struct definition {
            ...
            const boost::spirit::stored_rule<ScannerT>
            get_field_value_parser(const field_value::type_id field_type) const
            {
                ...
                case field_value::sfbool_id:
                    r = expect_bool(bool_p)[this->self.actions_.on_sfbool];
                    break;
                case field_value::sfcolor_id:
                    r = expect_color(color_p)[this->self.actions_.on_sfcolor];
                    break;
                case field_value::sffloat_id:
                    r = expect_float(float_p)[this->self.actions_.on_sffloat];
                    break;
                case field_value::sfimage_id:
                    r = expect_image(image_p)[this->self.actions_.on_sfimage];
                    break;
                case field_value::sfint32_id:
                    r = expect_int32(int32_p)[this->self.actions_.on_sfint32];
                    break;
                case field_value::sfnode_id:
                    r = this->sfnode(false)[this->self.actions_.on_sfnode];
                    break;
                case field_value::sfrotation_id:
                    r = expect_rotation(this->self.rotation_p)[
                            this->self.actions_.on_sfrotation
                        ];
                    break;
                case field_value::sfstring_id:
                    r = expect_string(string_p)[this->self.actions_.on_sfstring];
                    break;
                case field_value::sftime_id:
                    r = expect_float(boost::spirit::real_p)[
                            this->self.actions_.on_sftime
                        ];
                    break;
                case field_value::sfvec2f_id:
                    r = expect_vec2f(vec2f_p)[this->self.actions_.on_sfvec2f];
                    break;
                case field_value::sfvec3f_id:
                    r = expect_vec3f(vec3f_p)[this->self.actions_.on_sfvec3f];
                    break;
                case field_value::mfcolor_id:
                    r = mfcolor_p[this->self.actions_.on_mfcolor];
                    break;
                case field_value::mffloat_id:
                    r = mffloat_p[this->self.actions_.on_mffloat];
                    break;
                case field_value::mfint32_id:
                    r = mfint32_p[this->self.actions_.on_mfint32];
                    break;
                case field_value::mfnode_id:
                    r = mfnode_p[this->on_mfnode];
                    break;
                case field_value::mfrotation_id:
                    r = mfrotation_p[this->self.actions_.on_mfrotation];
                    break;
                case field_value::mfstring_id:
                    r = mfstring_p[this->self.actions_.on_mfstring];
                    break;
                case field_value::mftime_id:
                    r = mftime_p[this->self.actions_.on_mftime];
                    break;
                case field_value::mfvec2f_id:
                    r = mfvec2f_p[this->self.actions_.on_mfvec2f];
                    break;
                case field_value::mfvec3f_id:
                    r = mfvec3f_p[this->self.actions_.on_mfvec3f];
                    break;
                default:
                    assert(false);
                }
            ...
            struct set_field_value_rule_t {
                                ...
                                const IteratorT & last) const
                {
                    ...
                    d.self.actions_.on_field_start(interface_id,
                                                   pos->field_type);
                }
            ...
            struct on_scene_start_t {
                explicit on_scene_start_t(const Actions & actions):
                    actions(actions)
                {}
        ...
        vrml97_grammar(const node_type_decls & node_types,
                       const Actions & actions,
    ...
    private:
        ...
        const node_type_decls & node_types_;
    };
    ...
    definition(const vrml97_grammar & self):
        ...
        on_proto_interface(on_proto_interface_function(self.actions_)),
        on_proto_default_value_start(self.actions_),
        on_proto_default_value_finish(self.actions_),
        on_proto_finish(self.actions_),
        on_externproto(on_externproto_function(self.actions_)),
        on_node_start(on_node_start_function(self.actions_)),
        on_node_finish(self.actions_),
        on_script_interface_decl(
        ...
        node_type_already_exists(false)
    {
        ...
        BOOST_SPIRIT_DEBUG_NODE(interface_decl_p);
        ...
        BOOST_SPIRIT_DEBUG_NODE(mfnode_p);
