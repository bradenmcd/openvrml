...
long  Vrml97Parser::intValue() {
	...
	try {      // for error handling
		i = LT(1);
		match(INTEGER);
		istrstream(i->getText().c_str()) >> val;
	}
	catch (ANTLR_USE_NAMESPACE(antlr)RecognitionException& ex) {
		reportError(ex);
		consume();
